package net.timme7893.gangs.utils.file;
...
public class File {
    ...
    public ItemStack getItem(Player player, String section, PKeys pKeys, PValues pValues, String itemPlaceholder) {
        return ItemStackHelp.newItemStack(player, this.asString(section + ".item"),
                this.asString(section + ".name", pKeys, pValues), this.asList(section + ".lore", pKeys, pValues),
                itemPlaceholder);
    }
    ...
    public Long asLong(String section) {
        String replacer = "0";
        if (this.throwErrorCheck(section, replacer) || this.valueErrorCheck(section, "asLong", replacer))
            return 0l;
        return (Long) this.getCached(section);
    }
    ...
    public List<String> asList(String section, PKeys pKeys, PValues pValues) {
        ...
        ((List<String>) this.getCached(section))
                .forEach(string -> coloredList.add(Text.format(string, pKeys, pValues)));
        return coloredList;
    }
    ...
    private boolean valueErrorCheck(String section, String value, String replacer) {
        switch (value) {
        case "asLong":
            if (!this.getCached(section).getClass().getName().equals("java.lang.Long")) {
    ...
    private void printValueErrorCheck(String section, String value, String replacer) {
        System.out.println("==================================================================================");
        System.out.println("The configure section " + section + " has to be of the type "
                + value.toLowerCase().replace("as", "") + "!");
    ...
    private void cache() {
        this.fileMap = this.fileConfiguration.getKeys(true).stream()
                .collect(Collectors.toMap(key -> key, this.fileConfiguration::get));
    }

    /*
     * public FileWrapper getFileWrapper() { return fileWrapper; }
     */
